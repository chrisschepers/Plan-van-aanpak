...
// Voorbeeldcasus laden en verwerken
click($(".dropzone"));
await wait(50);
...
check("velden aanwezig (11)", $$(".field-row").length === 11);
check("2 velden ontbreken", $$(".field-row.missing").length === 2);
...
await wait(30);
check("PvA-tab toont Plan van Aanpak", !!byText(".doc-sheet h2", "Plan van Aanpak"));
check("PvA toont [INVULLEN] voor werkgever", !!byText(".doc-sheet", "[INVULLEN]"));
const dlBtn = byText("button", "Download als Word");
check("download-knop actief na controle", !dlBtn.disabled);
click(dlBtn);
await wait(30);
check("download-toast verschijnt", !!byText(".toast .tt", "gedownload"));
